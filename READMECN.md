...
这里下载了好几个杀毒软件来玩，俗称养蛊。腾讯管家希望下载个游戏中心被360挡了，360想改点什么被管家挡了，avast和AVG和无能的丈夫一样，啥都不干。下面是一个Wscript.exe的定时任务，每隔一段时间会启动powershell并且执行修改防火墙的指令。（不直接使用powershell定时是因为用来hidden还是会闪一下窗口出来）
```
